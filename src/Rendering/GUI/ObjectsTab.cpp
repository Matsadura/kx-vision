...
void RenderVisibleObjectsTable() {
 ...
 float dist = g->gameplayDistance; char buf[256];
 std::snprintf(buf, sizeof(buf), "%s - %.1fm", name.c_str(), dist);
 ImGui::TableNextRow(); ImGui::TableNextColumn(); ImGui::TextUnformatted(buf);
 }
 // Attack Targets
 for (const auto* a : filteredData.attackTargets) {
 if (!a) continue;
 std::string name = "Attack Target";
 float dist = a->gameplayDistance; char buf[128];
 std::snprintf(buf, sizeof(buf), "%s - %.1fm", name.c_str(), dist);
 ImGui::TableNextRow(); ImGui::TableNextColumn(); ImGui::TextUnformatted(buf);
 }
 } else {
 ...
 if (g && !g->name.empty()) name = g->name; else if (g) {
 if (const char* typeName = ESPFormatting::GetGadgetTypeName(g->type)) name = typeName; else name = "Gadget";
 } else name = "Gadget";
 } else name = "Attack Target";
 float dist = item.entity->gameplayDistance; char buf[256];
 std::snprintf(buf, sizeof(buf), "%s - %.1fm", name.c_str(), dist);
 ImGui::TableNextRow(); ImGui::TableNextColumn(); ImGui::TextUnformatted(buf);
 }
 }
...
void RenderObjectTypeFilters(ObjectEspSettings& settings) {
 ...
 ImGui::Indent();
 const float column1 =180.0f; const float column2 =360.0f;
...
void RenderDetailedInformationSettings(ObjectEspSettings& settings) {
 ...
 ImGui::Indent();
 CheckboxWithTooltip("Type##ObjectDetail", "ObjectDetails", &settings.showDetailGadgetType, "Show the type of gadget."); ImGui::SameLine();
